...
2	Robert B. Michael, Gregory Franco, Mevagh Sanson, Maryanne Garry	ONLINE MTURK - Michael, Franco, Sanson, Garry	Online	New Zealand	English	302	98	0	0	98	204	94	47	63	313	130	0	0	130	183	104	49	30
3	Victoria K. Alogna, Jamin Halberstadt, Jonathan Jong, Joshua C. Jackson, Cathy Ng	Alogna, Halbertstadt, Jong, Jackson, Ng	Both	New Zealand	English	70	20	0	20	0	50	25	7	18	67	17	0	17	0	50	31	15	4
...
7	Curt Carlson, Dawn Weatherford, Maria Carlson	Carlson, Weatherford, Carlson	Both	USA	English	81	6	4	0	2	75	32	26	17	79	4	3	0	1	75	50	12	13
...
13	Andre Kehn, Kimberly Schweitzer, Bradlee W. Gamblin, Kimberly Wiseman, Narina L. Nunez	Kehn, Schweitzer, Gamblin, Wiseman, Nunez	Both	USA	English	55	6	1	5	0	49	23	7	19	58	8	1	7	0	50	24	13	13
...
19	Christopher R. Poirier, Matthew K. Attaya, Kathleen A. McConnaughy, Jessica E. Pappagianopoulos, Griffin A. Sullivan	Poirer, Attaya, McConnaughy, Pappagianopoulos, Sullivan	Both	USA	English	46	2	0	1	1	44	13	13	18	49	8	0	0	8	41	24	9	8
...
res <- rma(measure="RD", ai=V_Hit, bi=V_Wrong, ci=C_Hit, di=C_Wrong, data=data, slab=paste(Author))
...
addpoly(repres, atransf=FALSE, row=-.7, cex=1.0, mlab="Meta-analytic effect for laboratory replications only")


#### Extending the graph (Here my addition to the code provided on OSF begins) ####
install.packages('fmsb', repos = 'http://cran.r-project.org/') 
require(fmsb)

# Checking that we can replicate the findings of the meta-analysis using the risk difference function in the fmsb package
# k <- 5
# riskdifference(V_Hit[k], C_Hit[k], V_Included[k], C_Included[k])
# We get a result that matches the 5th study of the forrent plot reported above. So what happens if we filter only studies that are 
# significant?

Pvals <- vector(mode = 'numeric', length = length(V_Hit))
for (i in 1:length(V_Hit)) {
  # Had to make this a bit more complicated to suppress unwanted output from riskdifference for Knitr
  Pvals[i] <- as.numeric(str_sub(capture.output(riskdifference(V_Hit[i], C_Hit[i], V_Included[i], C_Included[i])$p.value)[5], 5, 13))
}

### Adding effect size for significant studies only to the plot ###
# Reducing data used to significant studies (11 of 24, or 9 if you exclude the original and Mturk)
repdata <- data[3:24,]
repdata <- repdata[Pvals[3:24] <= .05,]
rownames(repdata) <- NULL

### run a meta-analysis on just the 9 significant laboratory replication studies. Note that I had to lower the step threshold to get this to
### converge with the original and Mturk included, but not without them.
repres <- rma(measure="RD", ai=repdata$N_verbal_correct, bi=(repdata$N_verbal_included - repdata$N_verbal_correct), 
              ci=repdata$N_control_correct, di=(repdata$N_control_included - repdata$N_control_correct), data=repdata, 
              control=list(threshold=10^-5, maxiter=10000))

### add a polygon to the Forest plot showing the meta-analytic effect of the replication studies
addpoly(repres, atransf=FALSE, row=-1.7, cex=1.0, col = "blue", mlab="Meta-analytic effect for significant replications only")
